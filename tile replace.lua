...
local main_tileindex_g = app.preferences.color_bar.fg_tile
local main_tileindex = app.pixelColor.tileI(main_tileindex_g)
local main_tileflags = app.pixelColor.tileF(main_tileindex_g)
local to_replace = {[main_tileindex] = true}
local replacement_tiles = {} -- 1 - tile, 2 - chance

--inclusive
local function get_ceils_index(ceils, num)
	for i = #ceils,1,-1 do
		local value = ceils[i]
		if value < num then
			return i + 1
...
local function get_replacement_tile(ceils, sum)
	local rand = math.random(sum)
	local index = get_ceils_index(ceils, rand)
	return replacement_tiles[index][1]
end
local function do_replacement()
	local ceils = {}
	local sum = 0
	for _,rt in ipairs(replacement_tiles) do
		sum = sum + rt[2]
		table.insert(ceils, sum)
	end

	local cel = app.cel
	if cel.image.colorMode == ColorMode.TILEMAP then
		local tilemap = cel.image
		local tilemap_copy = tilemap:clone()
		for it in tilemap:pixels() do
			local x = it.x
			local y = it.y
			local current_tileindex_g = it()
			local current_tileindex = pc.tileI(current_tileindex_g)
			local current_tileindex_flags = pc.tileF(current_tileindex_g)
			if to_replace[current_tileindex] then
				local replacement = get_replacement_tile(ceils, sum)
				local replacement_g = pc.tile(replacement, current_tileindex_flags)
				tilemap_copy:drawPixel(x, y, replacement_g)
			end
		end
		tilemap:drawImage(tilemap_copy)
		app.refresh()
	end
	Dialog("Done"):button{text="done"}:show({wait=false})
end

local function show_replacement_tile_dlg(wait, bounds)
	if wait == nil then
		wait = true
	end
	local dlg = Dialog("Replace")
	local to_replace_list = {}
	for k,_ in pairs(to_replace) do
		table.insert(to_replace_list, k)
	end
	dlg:label{ text = string.format("replace {%s}", table.concat(to_replace_list, ",")) }
	...
	dlg:button {
		...
		onclick = function ()
			dlg:close()
			local current_tileindex_g = app.preferences.color_bar.fg_tile
			local current_tileindex = app.pixelColor.tileI(current_tileindex_g)
			to_replace[current_tileindex] = true
			show_replacement_tile_dlg(false, dlg.bounds)
		end
	}
	dlg:button{
		id = "btn_add",
		...
		onclick = function ()
			...
			local current_tileindex_g = app.preferences.color_bar.fg_tile
			local current_tileindex = app.pixelColor.tileI(current_tileindex_g)
			table.insert(replacement_tiles, {current_tileindex, 10})
			show_replacement_tile_dlg(false, dlg.bounds)
		end
	}
	for i,rt in ipairs(replacement_tiles) do
		dlg:number {
			label = rt[1],
			id = tostring(i),
			text = tostring(rt[2]),
			onchange = function ()
				--warning will need change if replacement_tiles can change while this closure remains same.
				local new_chance = dlg.data[tostring(i)]
				if new_chance then
					rt[2] = new_chance
				end
			end
		...
		dlg:button {
			...
			onclick = function ()
				dlg:close()
				table.remove(replacement_tiles, i)
				show_replacement_tile_dlg(false, dlg.bounds)
			end
		}
	end
	if bounds then
		-- dlg.bounds = bounds
		local new_bounds = dlg.bounds
		dlg.bounds = Rectangle(bounds.x, bounds.y, new_bounds.w, new_bounds.h)
		-- dlg.bounds.x = bounds.x
		-- dlg.bounds.y = bounds.y
	end
	dlg:show({wait = wait})
end
show_replacement_tile_dlg(false)
if true then
	return
end
